...
function FlashcardPanel({
  ...
  // NEW: track selected option and whether answer was submitted
  ...
  const handleOptionSelect = (opt: string) => {
    ...
    setAnswered(true);
    // auto-mark: correct = known, wrong = review
    const isCorrect = opt === current.answer;
    setTimeout(() => {
      markCard(isCorrect ? "known" : "review");
    }, 1400);
  };
    ...
    <div className="mt-7 rounded-[28px] border border-violet-500/15 bg-gradient-to-br from-violet-500/10 via-background to-blue-500/10 p-5 md:p-6 shadow-[0_10px_40px_rgba(139,92,246,0.08)]">
      {!showSummary ? (
        <>
          ...
          {/* Flashcard — MODIFIED: no flip on click when not answered */}
          <div
          ...
          >
            ...
            >
              {/* Front — question side, now shows options */}
              <div
              ...
              >
                <div className="flex flex-col gap-4 p-7 md:p-9">
                  ...
                  {/* NEW: Options rendered on the front face */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-1">
                    {current.options.map((opt) => {
                      ...
                      let optClass =
                        "rounded-xl border px-4 py-3 text-sm font-medium text-left transition-all duration-200 cursor-pointer ";

                      if (!answered) {
                        optClass +=
                          "border-violet-500/15 bg-background hover:border-violet-500/40 hover:bg-violet-500/5 text-foreground";
                      } else if (isCorrect) {
                        optClass +=
                        ...
                        optClass +=
                          "border-violet-500/10 bg-background/50 text-muted-foreground opacity-60";
                      }
                        ...
                        >
                          <span className="flex items-center gap-2">
                            ...
                            {answered && isSelected && !isCorrect && (
                              // X icon — use a simple ✕ since no X icon imported
                              <span className="h-3.5 w-3.5 shrink-0 text-red-500 font-bold text-base leading-none">✕</span>
                            )}
                            {opt}
                          </span>
                        </button>
                  ...
                  {/* NEW: explanation after answering */}
                  {answered && (
                    <div className="rounded-2xl border border-violet-500/10 bg-background/60 p-4 mt-1">
              ...
              {/* Back face — kept but unused now (flip removed) */}
              <div
                className="absolute inset-0 w-full rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 via-background to-blue-500/10"
                style={{
