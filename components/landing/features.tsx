...
function QuizPanel({
  ...
  const explanation =
    currentQuestion && feature
      ? getExplanation(feature, currentQuestion, currentIndex)
      : "";
    ...
    <div className="mt-7 rounded-[28px] border border-violet-500/15 bg-gradient-to-br from-violet-500/10 via-background to-blue-500/10 p-5 md:p-6 shadow-[0_10px_40px_rgba(139,92,246,0.08)]">
      {!showResult ? (
        <>
          ...
          {/* Progress */}
          <div className="mt-5 h-2 w-full rounded-full bg-violet-500/10">
            <div
          ...
          {/* Question */}
          <div className="mt-6 rounded-3xl border border-violet-500/10 bg-background/80 p-5">
                  ...
                  <button
                    ...
                    className={`rounded-2xl border px-4 py-3 text-left transition ${
                      showFeedback
                        ...
                        : "border-violet-500/10 hover:bg-violet-500/5"
                    }`}
                  >
                    {option}
                  </button>
...
function SortingPreview() {
  ...
  return (
    <PreviewShell
      label="Animated Bars"
      rightLabel="Sorting Flow"
      footer="Compare, swap, and reorder visually"
    >
      <div className="flex h-full items-end justify-center gap-2 pt-5 pb-3">
        {bars.map((h, i) => (
          <div
            key={i}
            className={`w-5 rounded-t-[14px] bg-gradient-to-t ${
              i === 3
                ? "from-amber-500 via-fuchsia-500 to-violet-600"
                : "from-violet-600 via-violet-500 to-blue-500"
...
function StackPreview() {
  return (
    <PreviewShell
      label="LIFO"
      rightLabel="Push / Pop"
      footer="Last item enters and exits first"
    >
      <div className="flex h-full items-center justify-center pt-3">
        <div className="relative flex w-24 flex-col-reverse gap-2 rounded-[20px] border border-violet-500/15 bg-white/40 p-2 dark:bg-white/[0.03]">
          {["10", "18", "27", "44"].map((item, i) => (
            <div
              key={item}
              className={`flex h-10 items-center justify-center rounded-xl border border-violet-500/15 text-sm font-semibold transition-all duration-500 ${
                i === 3
                  ? "bg-gradient-to-r from-amber-400/30 to-violet-500/20 group-hover:-translate-y-2"
...
function QueuePreview({ message = false }: { message?: boolean }) {
  ...
  return (
    <PreviewShell
      label="FIFO"
      rightLabel={message ? "Producer / Consumer" : "Queue Flow"}
      footer={
        message
        ...
        <div className="w-full rounded-[22px] border border-violet-500/15 bg-white/40 p-3 dark:bg-white/[0.03]">
          <div className="flex items-center gap-2 overflow-hidden">
            {items.map((item, i) => (
              <div
                key={item}
                className={`min-w-[68px] rounded-xl border px-3 py-4 text-center text-sm font-semibold transition-all duration-500 ${
                  i === 0
                    ? "border-amber-400/25 bg-gradient-to-r from-amber-400/25 to-violet-500/10 group-hover:-translate-x-2"
                    : i === items.length - 1
                    ? "border-violet-500/15 bg-gradient-to-r from-violet-500/15 to-blue-500/10 group-hover:translate-x-2"
                    : "border-violet-500/15 bg-gradient-to-r from-violet-500/15 to-blue-500/10"
                }`}
              >
                {item}
              </div>
...
function LinkedListPreview() {
  return (
    <PreviewShell
      label="Node Links"
      rightLabel="Pointer Chain"
      footer="Traverse node to node with links"
    >
      <div className="flex h-full items-center justify-center pt-3">
        <div className="flex items-center gap-2">
          {["10", "22", "31", "47"].map((node, i) => (
            <div key={node} className="flex items-center gap-2">
              <div className="flex h-12 w-12 items-center justify-center rounded-full border border-violet-500/20 bg-gradient-to-br from-violet-500/20 to-amber-400/20 text-sm font-semibold">
                {node}
              </div>
              {i < 3 && (
...
function BSTPreview() {
  return (
    <PreviewShell
      label="BST View"
      rightLabel="Ordered Search"
      footer="Left smaller, right larger"
    >
      <div className="relative mx-auto h-full max-w-[220px] pt-2">
        <div className="absolute left-1/2 top-2 flex h-10 w-10 -translate-x-1/2 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-blue-500 text-sm font-semibold text-white">
          40
        </div>

        <div className="absolute left-[28%] top-14 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/75 text-sm font-semibold dark:bg-white/[0.06]">
          22
        </div>
        <div className="absolute right-[28%] top-14 flex h-10 w-10 items-center justify-center rounded-full border border-amber-400/25 bg-amber-400/15 text-sm font-semibold">
          61
        </div>

        <div className="absolute left-[13%] top-28 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/75 text-sm font-semibold dark:bg-white/[0.06]">
          12
        </div>
        <div className="absolute left-[41%] top-28 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/75 text-sm font-semibold dark:bg-white/[0.06]">
          31
        </div>
        <div className="absolute right-[41%] top-28 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/75 text-sm font-semibold dark:bg-white/[0.06]">
          52
        </div>
        <div className="absolute right-[13%] top-28 flex h-10 w-10 items-center justify-center rounded-full border border-amber-400/25 bg-amber-400/15 text-sm font-semibold">
          74
        </div>
...
function AVLPreview() {
  return (
    <PreviewShell
      label="Balanced Tree"
      rightLabel="AVL Rotations"
      footer="Rebalance with smart rotations"
    >
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 pt-2">
          <div className="rounded-2xl border border-red-400/20 bg-red-400/10 px-4 py-3 text-center">
            ...
            <div className="mt-2 space-y-2 text-sm font-semibold">
              <div className="mx-auto flex h-9 w-9 items-center justify-center rounded-full bg-violet-500/20">
                30
              </div>
              <div className="mx-auto flex h-9 w-9 items-center justify-center rounded-full bg-violet-500/20">
                20
              </div>
              <div className="mx-auto flex h-9 w-9 items-center justify-center rounded-full bg-amber-400/25">
                10
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-center">
            ...
            <div className="mt-2 flex items-center justify-center gap-2 text-sm font-semibold">
              ...
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-amber-400/25">
                30
              </div>
            </div>
          </div>
...
function HeapPreview() {
  return (
    <PreviewShell
      label="Priority Heap"
      rightLabel="Max Heap"
      footer="Highest priority stays on top"
    >
      <div className="relative mx-auto h-full max-w-[220px] pt-2">
        <div className="absolute left-1/2 top-2 flex h-10 w-10 -translate-x-1/2 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-blue-500 text-sm font-semibold text-white">
          90
        </div>
        <div className="absolute left-[30%] top-14 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/75 text-sm font-semibold dark:bg-white/[0.06]">
          65
        </div>
        <div className="absolute right-[30%] top-14 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/75 text-sm font-semibold dark:bg-white/[0.06]">
          52
        </div>
        <div className="absolute left-[14%] top-28 flex h-10 w-10 items-center justify-center rounded-full border border-amber-400/25 bg-amber-400/15 text-sm font-semibold">
          31
        </div>
        <div className="absolute right-[14%] top-28 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/75 text-sm font-semibold dark:bg-white/[0.06]">
          18
        </div>
...
function ExpressionPreview() {
  return (
    <PreviewShell
      label="Expression Flow"
      rightLabel="Stack Logic"
      footer="Convert symbols using precedence rules"
    >
      <div className="flex h-full flex-col justify-center gap-3 pt-1">
        <div className="rounded-2xl border border-violet-500/15 bg-white/55 px-4 py-3 text-center text-sm font-semibold tracking-wide dark:bg-white/[0.03]">
          ( A + B ) × C
        </div>
        <div className="flex items-center justify-center gap-2 text-xs font-semibold text-muted-foreground">
          <span className="rounded-full bg-violet-500/10 px-2 py-1 text-violet-700 dark:text-violet-300">
            scan
          </span>
          <ArrowRight className="h-3.5 w-3.5" />
          <span className="rounded-full bg-amber-400/15 px-2 py-1 text-amber-700 dark:text-amber-300">
            stack
          </span>
          <ArrowRight className="h-3.5 w-3.5" />
          <span className="rounded-full bg-blue-500/10 px-2 py-1 text-blue-700 dark:text-blue-300">
            output
          </span>
        </div>
        <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-center text-sm font-semibold">
          AB+C×
        </div>
      </div>
    </PreviewShell>
...
function PolynomialPreview() {
  return (
    <PreviewShell
      ...
      rightLabel="Term Multiplication"
      footer="Break larger expressions into term pairs"
    >
      <div className="flex h-full flex-col justify-center gap-3 pt-1">
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-xl border border-violet-500/15 bg-violet-500/10 px-3 py-3 text-center text-sm font-semibold">
            2x² + 3x
          </div>
          <div className="rounded-xl border border-amber-400/20 bg-amber-400/10 px-3 py-3 text-center text-sm font-semibold">
            x + 4
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2 text-sm font-semibold">
          <div className="rounded-xl border border-violet-500/15 bg-white/60 px-2 py-2 text-center dark:bg-white/[0.03]">
            2x³
          </div>
          <div className="rounded-xl border border-violet-500/15 bg-white/60 px-2 py-2 text-center dark:bg-white/[0.03]">
            8x²
...
function HuffmanPreview() {
  return (
    <PreviewShell
      label="Encoding Tree"
      rightLabel="Compression"
      footer="Shorter codes for frequent symbols"
    >
...
function GraphPreview() {
  return (
    <PreviewShell
      label="Shortest Path"
      rightLabel="Weighted Graph"
      footer="Choose the lightest total route"
    >
      <div className="relative mx-auto h-full max-w-[240px] pt-2">
        <div className="absolute left-2 top-16 flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-blue-500 text-sm font-semibold text-white">
          A
        </div>
        <div className="absolute left-24 top-4 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/70 text-sm font-semibold dark:bg-white/[0.06]">
          B
        </div>
        <div className="absolute left-24 top-28 flex h-10 w-10 items-center justify-center rounded-full border border-violet-500/20 bg-white/70 text-sm font-semibold dark:bg-white/[0.06]">
          C
        </div>
        <div className="absolute right-2 top-16 flex h-10 w-10 items-center justify-center rounded-full border border-amber-400/25 bg-amber-400/15 text-sm font-semibold">
          D
        </div>
        ...
        <div className="absolute left-[78px] top-[42px] rounded-full bg-white px-1.5 text-[10px] font-bold dark:bg-zinc-900">
          4
        </div>
        <div className="absolute left-[78px] top-[95px] rounded-full bg-white px-1.5 text-[10px] font-bold dark:bg-zinc-900">
          2
        </div>
        <div className="absolute left-[154px] top-[44px] rounded-full bg-white px-1.5 text-[10px] font-bold dark:bg-zinc-900">
          3
        </div>
        <div className="absolute left-[154px] top-[91px] rounded-full bg-white px-1.5 text-[10px] font-bold dark:bg-zinc-900">
          6
        </div>
      </div>
    </PreviewShell>
...
const PreviewCard = ({
  ...
  type: SelectedFeature["previewType"];
}) => {
  switch (type) {
    case "sorting":
    ...
    case "message-queue":
      return <QueuePreview message />;
    case "polynomial":
      return <PolynomialPreview />;
    case "huffman":
      return <HuffmanPreview />;
    case "graph":
      return <GraphPreview />;
    default:
      return <SortingPreview />;
...
>
<DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto border-violet-500/15 bg-background/95 backdrop-blur-2xl rounded-3xl p-0">          {selectedFeature && (
            <div className="relative">
  ...
  <div className="mt-7 grid grid-cols-1 lg:grid-cols-12 gap-5">
    <div className="lg:col-span-7 rounded-3xl border border-violet-500/15 bg-gradient-to-br from-violet-500/10 via-background to-blue-500/10 p-5 md:p-6 shadow-[0_10px_40px_rgba(139,92,246,0.08)]">
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
        {selectedFeature.learnPoints.map((point) => (
          <div
            key={point}
            className="rounded-2xl border border-violet-500/10 bg-background/80 px-4 py-4 text-sm font-medium shadow-sm"
          >
            {point}
          </div>
        ))}
    ...
    <div className="lg:col-span-5 flex flex-col gap-4">
      <div className="rounded-3xl border border-violet-500/15 bg-background/80 p-5 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2">
          {selectedFeature.useCases.map((item) => (
            <span
              key={item}
              className="rounded-full border border-violet-500/10 bg-violet-500/5 px-3 py-2 text-sm"
            >
              {item}
            </span>
          ))}
      ...
      <div className="rounded-3xl border border-violet-500/15 bg-gradient-to-br from-fuchsia-500/10 via-transparent to-blue-500/10 p-5 shadow-sm">
        ...
        <p className="text-sm md:text-base leading-7 text-muted-foreground">
          Visual learning makes operations easier to understand.
          Instead of memorizing steps, you can watch each action,
  ...
  >
    {showQuiz ? "Close Quiz" : "Close Preview"}
  </Button>
...
>
  ...
  >
    Try with Code <Code2 className="h-4 w-4" />
      ...
      >
        <Link
          href={`${selectedFeature.url}?mode=input`}
          className="gap-2"
        >
