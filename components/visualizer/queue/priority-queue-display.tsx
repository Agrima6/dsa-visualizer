...
export function PriorityQueueDisplay({
  queue,
  highlightedIndex,
}: {
  queue: QueueNode[]
  ...
  return (
    <div className="relative overflow-hidden rounded-[28px] border border-violet-500/15 bg-[linear-gradient(145deg,rgba(255,255,255,0.96),rgba(245,243,255,0.94)_34%,rgba(255,248,235,0.92)_100%)] p-6 shadow-[0_10px_40px_rgba(139,92,246,0.08)] backdrop-blur-xl dark:bg-[linear-gradient(145deg,rgba(20,18,30,0.96),rgba(17,14,27,0.98)_34%,rgba(34,24,10,0.72)_100%)] dark:shadow-[0_16px_50px_rgba(0,0,0,0.28)]">
      ...
      <div className="relative flex min-h-[280px] w-full flex-col justify-center">
        <div className="mb-5 flex items-center gap-2 px-2 text-xs font-semibold uppercase tracking-wider text-violet-700 dark:text-violet-300">
                  ...
                  {queue.map((node, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`relative flex h-20 w-20 flex-shrink-0 flex-col items-center justify-center rounded-xl border will-change-transform ${
                          isActive
                      ...
                      >
                        {isTop && <Crown className="absolute -top-2.5 h-4 w-4 text-emerald-500" />}
                        <span className="text-lg font-mono font-semibold">{node.value}</span>
                        <span className="text-[9px] text-muted-foreground/70">priority</span>
                      </motion.div>
                    )
                  })}
                </AnimatePresence>
