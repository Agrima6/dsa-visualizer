...
console.log(add5(2));`,
    ...
    answerIndex: 0,
    explanation: "The inner function closes over x=5. Calling add5(2) computes 5 + 2 = 7 — the closure remembered x even after makeAdder returned.",
  },
  {
    id: "q6",
    code: `function addItem(arr) {
  arr.push("new");
}
const list = ["a", "b"];
addItem(list);
console.log(list.length);`,
    ...
    answerIndex: 0,
    explanation: "Arrays and objects are passed by reference — addItem mutates the SAME array the caller owns, so the push is visible outside the function too.",
  },
  {
    id: "q7",
    code: `function transform(arr, fn) {
  return arr.map(fn);
}
console.log(transform([1, 2, 3], n => n * 10));`,
    options: ["[10, 20, 30]", "[1, 2, 3]", "10", "Error"],
    answerIndex: 0,
    explanation: "transform is a higher-order function — it takes the callback fn and applies it to every element via .map().",
  },
  {
    id: "q8",
    code: `let count = 1;
